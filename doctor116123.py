...
class residents:
    ...
    def fetch_data(self):
        con=mysql.connector.connect(host="localhost",user="root",password="12345",database="residents")
        cur = con.cursor()
    ...
    def fetchDeath_data(self):
        con=mysql.connector.connect(host="localhost",user="root",password="12345",database="residents")
        cur = con.cursor()
        cur.execute("select r.Name ,r.SplId,d.DeathDate,e.hosId,e.hosName,c.Place_id,c.PLace_Name from hospitals e,residents r,deaths d,quarantine_center c where r.`SplId`=d.splId and d.hosId=e.hosId and d.Place_Id=c.Place_id")
        rows = cur.fetchall()
        if len(rows) != 0:
    ...
    def fetchRecovered_data(self):
        con=mysql.connector.connect(host="localhost",user="root",password="12345",database="residents")
        cur = con.cursor()
        cur.execute("select a.`SplId`,a.Name,b.RecovDate,b.Place_Id,c.PLace_name,b.hosId,d.hosName from residents a,recovered b,quarantine_center c,hospitals d where a.`SplId`=b.splid and b.Place_Id=c.Place_id and b.hosId=d.hosId")
        rows = cur.fetchall()
        if len(rows) != 0:
    ...
    def fetchQuarantine_data(self):
        con=mysql.connector.connect(host="localhost",user="root",password="12345",database="residents")
        cur = con.cursor()
        cur.execute("select a.`SplId`,a.Name,b.startDate,b.endDate,b.Place_Id,c.PLace_name from residents a,quarantined b,quarantine_center c where a.`SplId`=b.splid and b.Place_Id=c.Place_id ")
        rows = cur.fetchall()
        if len(rows) != 0:
    ...
    def searchRecovered_data(self):
        ...
        cur.execute("select a.`SplId`,a.Name,b.RecovDate,b.Place_Id,c.PLace_name,b.hosId,d.hosName from residents a,recovered b,quarantine_center c,hospitals d where a.`SplId`=b.splid and b.Place_Id=c.Place_id and b.hosId=d.hosId and a.`"+ str(self.Recovered_Value.get())+"` like '%" + str(self.Recovered_Text.get()) + "%'")
        rows1 = cur.fetchall()
        if len(rows1) != 0:
            self.Recovered_table.delete(*self.Recovered_table.get_children())
            for row in rows1:
    ...
    def searchQuarantine_data(self):
        ...
        cur.execute("select a.`SplId`,a.Name,b.startDate,b.endDate,b.Place_Id,c.PLace_name from residents a,quarantined b,quarantine_center c where a.`SplId`=b.splid and b.Place_Id=c.Place_id and a.`"+ str(self.Quarantined_Value.get())+"` like '%" + str(self.Quarantined_Text.get()) + "%'")
        rows1 = cur.fetchall()
        print()
        if len(rows1) != 0:
            self.Quarantine_table.delete(*self.Quarantine_table.get_children())
